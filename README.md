Account & Auth:

1.As a visitor, I want to sign up for an account with an email and password, so that I can save builds and place orders.
2.As a user, I want to log in and log out, so that my account stays secure.
.

Browsing PCs:

1.As a visitor, I want to see a list of all PCs with name, price, and image, so that I can browse what's available.
2.As a visitor, I want to click on a PC to see full details (specs, price, description), so that I can decide if it fits my needs.
3.As a visitor, I want to filter or sort PCs by price, use case (gaming, workstation, budget), or brand, so that I can find one that matches my needs faster.



Account: 
.As a logged-in user, I want to view and edit my profile (name, email, shipping address), so that my order info stays accurate

[ERD](https://drive.google.com/drive/home?dmr=1&ec=wgc-drive-%5Bmodule%5D-goto)

[WireFrame](https://excalidraw.com/#json=uggb83N0LoRqebVMxq_ny,KBlERNrMnxACWo7Naq3n5g)